...
def get_save_path(args):
    # savedirs=[f'{args.savedir}{args.model}/{v.lower().split('_detrend')[0]}/{args.experiment}/' for v in args.variables]
    savedirs=[(args.savedir+args.model+"/"+v.lower().split('_detrend')[0]
               +"/"+args.experiment+"/") for v in args.variables]
    for savedir in savedirs:
        print(savedir)
        os.makedirs(savedir,exist_ok=True)
        print("Exists after makedirs:", os.path.isdir(savedir))
